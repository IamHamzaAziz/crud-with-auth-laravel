...
<body class="bg-[#f0f0f0] m-0 p-0">
    ...
    <form action="/post/{{ $post->id }}" method="POST" class="w-4/5 mx-auto flex flex-col items-center space-y-4">
        @csrf
        @method('PUT')
        <input type="text" name="title" placeholder="Title" value="{{ $post->title }}" class="w-full p-3 rounded border border-gray-300 focus:outline-none focus:border-blue-500">
        <textarea name="body" placeholder="Body" class="w-full p-3 rounded border border-gray-300 focus:outline-none focus:border-blue-500">{{ $post->body }}</textarea>
